            ...
            <div className={`left w-[${isExpanded ? "100%" : "50%"}]`}>
                <div className="tabs flex items-center justify-between gap-2 w-full bg-[#1A1919] h-[50px] px-[40px]">
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <i className='features text-[20px] cursor-pointer' onClick={()=>(setIsExpanded(!isExpanded))}><AiOutlineExpandAlt /></i>
                  </div>

                </div>
                <Editor height="83vh" theme={islightMode ? "vs-light" : "vs-dark"}  defaultLanguage="javascript" defaultValue="// some comment" />
            </div>
            <iframe id='output' className={`w-[${isExpanded ? "0%" : "50%"}] ${isExpanded ? "hidden" : ""} min-h-[89vh] bg-[#fff] text-black`}>iframe</iframe>
        </div>
    </>
  )
}
